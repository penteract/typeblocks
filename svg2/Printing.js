...
function printRec(g) {
  ...
  if (g.isHole) {
    ...
    let n = 0
    let infix = false
    for (let node of g.children) if (node.nodeName.toLowerCase() !== "path") {// This makes some assumptions about text nodes (eg there's only 1)
      n++
...
function buildTerm(g) {
  ...
  if (g.isHole) {
    ...
    for (let node of g.children) if (node.nodeName.toLowerCase() !== "path") {
      if (node.nodeName.toLowerCase() === "text") {
        //if (term.head) throw "More than 1 text node";
        if (!term.head) term.head = new Constant(node.innerHTML)
      }
      else {
